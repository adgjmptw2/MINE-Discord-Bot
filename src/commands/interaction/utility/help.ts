...
function linesCoinSummary(): string[] {
  return [
    ...
    "`/내검` — 내 검 강화 상태를 확인합니다.",
    "`/강화` — 코인을 사용해 검 강화를 시도합니다.",
    "`/강화정보` — 다음 강화 비용과 확률을 확인합니다.",
    "`/던전` — 하루 한 번 검 강화 수치에 따른 코인을 획득합니다.",
    "`/칭호장착` — 보유한 칭호를 장착합니다.",
    "`/칭호해제` — 현재 칭호를 해제합니다.",
    "`/자산` — 내 총 잔액을 확인합니다.",
...
function buildStockDetail(): string[] {
  return [
    ...
    "`/시세` — 종목별 시세·1일 라인 그래프를 봅니다(채널 공개).",
    "`/매수` — 코인으로 매수합니다(채널 공개).",
    "`/매도` — 금액·비율·전부 등으로 매도합니다(채널 공개).",

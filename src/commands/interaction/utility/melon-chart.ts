...
const command: SlashCommand = {
  ...
  description:
    "Set the global YouTube playlist for [인기차트] on every server (bot developers only).",
  ...
  async run(_client, interaction) {
    ...
    if (sub === "clear") {
      ...
      await interaction.reply({
        content:
          "전역 인기차트 재생목록 등록을 지웠습니다. 모든 서버에서 [인기차트]가 비어 있습니다.",
        flags: MessageFlags.Ephemeral,
      });
      scheduleEphemeralReplyDelete(interaction);
      return;

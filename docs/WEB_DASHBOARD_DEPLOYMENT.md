...
- **공개 홈(`/`, `/index.html`)**: `src/web/homePage.ts`가 **로그인 없이** HTML을 반환합니다. **웹 리모컨 열기** → `/dashboard`, **개인정보처리방침** → `/privacy`, **이용약관** → `/terms`. `WEB_DASHBOARD_PUBLIC_URL`이 있으면 canonical·`og:url`에 사용합니다. `WEB_DASHBOARD_INVITE_URL`이 없으면 OAuth Client ID로 초대 URL을 만들고, 둘 다 없으면 **초대 링크 준비 중**으로 표시합니다. GitHub·지원 서버 링크는 해당 env가 있을 때만 표시합니다. 외부 링크는 **http(s)만** 허용합니다. **새 API·DB 없음**.
- **웹 리모컨 레이아웃(`/dashboard`)**: **서버 사이드바**(데스크톱 접기/펼치기·모바일 드로어)와 **메인 작업 영역**(현재 곡·조작)·작업 탭·대기열 내부 스크롤. 홈 페이지 CSS는 dashboard·legal에 영향을 주지 않습니다. 배포 후 **https://soundroom.minebot.org/** · **/dashboard** · **/privacy** · **/terms** · **/health** 를 확인하세요.
...
- **저장형 플레이리스트** (`/api/auth/playlists/…`): 목록·상세 DTO에 **재생 N회**(`queueAddCount`, 대기열 추가 성공 횟수)·**즐겨찾기 N명**(`favoriteCount`, COUNT만)·`lastQueuedAt`이 포함됩니다. **조회수(view count)는 없습니다.** 새 API 경로는 추가하지 않았습니다. 웹 리모컨 **플레이리스트** 섹션에서 내 목록·공개 목록·**즐겨찾기**를 조회하고, 곡(링크·메타데이터만, 파일 업로드 없음)을 관리할 수 있습니다. 각 목록 탭에서 **검색·정렬**(클라이언트)을 사용할 수 있으며, 공개·즐겨찾기 목록에서는 상세를 열지 않고 **대기열에 바로 추가**할 수 있습니다. 즐겨찾기 해제는 원본 플레이리스트를 삭제하지 않습니다. 공개·즐겨찾기 목록에는 `ownerUserId`가 아닌 표시용 이름(`ownerNameSnapshot`)만 노출됩니다. 플레이리스트당 최대 50곡, 유저당 20개·공개 5개. **즐겨찾기**: 로그인 사용자는 공개 플레이리스트를 개인별로 저장(`GET …/favorites`, `POST`/`DELETE …/:id/favorite`, CSRF는 쓰기만). 즐겨찾기는 수정·삭제 권한을 주지 않으며, 숨김·삭제된 공개 플레이리스트는 즐겨찾기 목록에 나타나지 않습니다. 즐겨찾기/해제는 Discord 채널 안내를 보내지 않습니다. **현재 대기열에 추가**는 `POST …/soundroom/playlists/:id/add-to-queue`(10/25/50곡 선택, 최대 50곡, CSRF·노래채널 권한). 수정·삭제·곡 관리 UI는 `canManage`가 true일 때만 표시(작성자 또는 `DISCORD_OWNER_IDS` 봇 운영자). **공개 플레이리스트 신고**: 로그인 사용자는 공개 플레이리스트 상세에서 `POST …/playlists/:id/report`로 신고할 수 있습니다(자기 플레이리스트·중복 신고 불가, 신고 횟수로 자동 숨김 없음). **운영자 관리**: `DISCORD_OWNER_IDS`만 `GET …/admin/public`, `GET …/admin/reports`, `POST …/admin/hide`, `POST …/admin/reports/:id/resolve`를 사용하며 **운영자** 탭(공개 관리·신고 목록)에서 숨김/해제·처리 완료를 할 수 있습니다. 신고·처리·숨김/해제는 Discord 채널 안내를 보내지 않으며, 신고/관리 UI에 `reporterUserId`·`ownerUserId` 원문은 표시하지 않습니다. 대기열 추가 성공 시 노래채널 안내 1건·30초 삭제·`allowedMentions: { parse: [] }` 정책은 기존 웹 리모컨과 동일합니다.

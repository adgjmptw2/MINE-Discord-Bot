...
- **저장형 플레이리스트** (`/api/auth/playlists/…`): 웹 리모컨 **플레이리스트** 섹션에서 내 목록·공개 목록을 조회하고, 곡(링크·메타데이터만, 파일 업로드 없음)을 관리할 수 있습니다. 공개 목록에는 `ownerUserId`가 아닌 표시용 이름(`ownerNameSnapshot`)만 노출됩니다. 플레이리스트당 최대 50곡, 유저당 20개·공개 5개. **현재 대기열에 추가**는 `POST …/soundroom/playlists/:id/add-to-queue`(10/25/50곡 선택, 최대 50곡, CSRF·노래채널 권한). 수정·삭제·곡 관리 UI는 `canManage`가 true일 때만 표시(작성자 또는 `DISCORD_OWNER_IDS` 봇 운영자). 공개 숨김(admin hide)은 웹 UI에 없으며 봇 운영자 API만 제공합니다. 대기열 추가 성공 시 노래채널 안내 1건·30초 삭제·`allowedMentions: { parse: [] }` 정책은 기존 웹 리모컨과 동일합니다. 목록·상세·생성·수정·삭제·곡 추가/삭제/순서 변경 자체는 Discord 안내를 보내지 않습니다.

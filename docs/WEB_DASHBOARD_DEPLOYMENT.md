...
- **웹 리모컨 레이아웃**: `/dashboard`는 **서버 사이드바**(데스크톱 접기/펼치기·모바일 드로어, 선택 시 데스크톱 자동 접힘)와 **메인 작업 영역**(현재 곡 hero·조작)으로 구성됩니다. **노래 추가 · 플레이리스트 · 대기열** 작업 탭은 `role="tablist"`/`tab`/`tabpanel`·`aria-selected`·focus-visible를 사용하며, 대기열은 탭 안 **내부 스크롤**(`queue-scroll-panel`)로 긴 목록을 처리합니다. **70단계 마감**: 모바일 **360·390·768px**, 데스크톱 **1024px** 이상에서 레이아웃·말줄임·접근성을 수동 확인하는 것을 권장합니다. **새 API·DB 변경 없음**.

...
- [ ] **레이아웃(70단계 마감)**: 서버 사이드바 접기/펼치기(`aria-expanded`·localStorage)·모바일 드로어(서버명 버튼·백드롭)·현재 곡 hero(긴 제목 말줄임·대기 중 안내)·작업 탭(`aria-selected`·focus-visible)·대기열 50곡+ 내부 스크롤·마지막 항목 버튼 잘림 없음
- [ ] **반응형**: **360px·390px·768px·1024px** 이상에서 hero·탭·조작·대기열·플레이리스트 카드가 깨지지 않음 (새 API·DB 없음)
- [ ] **노래 검색**
- [ ] **노래 추가** (단일 곡)

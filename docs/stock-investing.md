...
길드(서버)마다 가상 화폐와 지갑·보유·거래 기록을 두고, 국내 대형주 5종을 사고팔 수 있는 모의투자 미니 게임입니다.

실제 돈·환전·현물 보상은 없으며 실제 투자 판단에 사용할 수 없습니다.
...
기본값은 5분이며 설정에서 최소 간격 보정이 있을 수 있습니다.
...
보유 감소 후 0이면 행 삭제, 지갑 증가, stock_trades에 SELL을 기록합니다.
...
`/주식랭킹`은 길드 전량 조회 후 메모리 집계이므로 대규모 서버에서는 최적화 여지가 있습니다.

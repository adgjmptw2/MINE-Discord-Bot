...
**`/상점`**·**`/구매`**는 가상 코인으로 **칭호(TITLE)** 아이템을 사고, **`/내아이템`**으로 보유·장착 상태를 확인합니다. **`/칭호장착`**·**`/칭호해제`**로 길드당 칭호 하나를 장착하거나 뺄 수 있으며, 장착 정보는 `coin_equipped_items`(PK: 길드·유저·`item_type`)에 저장됩니다. **`/프로필`**은 (옵션으로 다른 유저 지정 가능) 장착 칭호·총자산·현금·주식 평가·보유 아이템 수·업적 진행도(완료 수/전체)·최근 미니게임 1건·현재 시즌 이름을 한 패널에 모아 보여 줍니다. 상품 목록은 코드 상수(`src/settings/coinShopItems.ts`)이며, 구매 시 `cash_balance`만 차감하고 **`total_deposit`은 바꾸지 않습니다.** 칭호는 **스탯·효과 없이 표시용**이며, **`/자산`**·**`/랭킹`**에 장착 칭호가 붙어 보입니다. 보유는 `coin_inventory_items`에 저장됩니다.

**`/업적`**은 출석·알바·낚시·가위바위보·주식 거래·상점 구매·칭호 장착 등 **기존 활동 로그**를 조회해 업적 달성 여부를 계산하고, 달성한 업적마다 **1회성 코인 보상**을 받을 수 있습니다. 보상 수령 여부는 `coin_achievement_rewards`(PK: 길드·유저·`achievement_key`)에만 저장되며, 보상 지급 시 `cash_balance`와 `total_deposit`이 함께 증가합니다. 업적 정의는 `src/settings/coinAchievements.ts`에 고정되어 있습니다.
...
| /칭호해제 | 장착 칭호 해제 |
| /프로필 | 코인 프로필(칭호·자산·아이템 수·업적 진행도·최근 게임·시즌, 공개) |
| /업적 | 업적 목록·진행도·보상 수령(ephemeral, 옵션 `보상받기:true`로 일괄 수령) |
| /자산 | 총자산(현금+주식 평가), 장착 칭호가 있으면 제목에 `[칭호]` 표시 |
| /주식자산 | 현금·보유·평가·총자산·수익률(본인만, ephemeral) |
| /주식목록 | 지원 5종 + 캐시 시세 (ANSI 색상·공개 메시지) |
| /시세 | 종목별 캐시 시세(채널 공개) |
| /매수 | 캐시 가격 기준 매수·수수료·거래 기록(채널 공개, `.env`로 거래 시간 제한 가능) |
| /매도 | 금액·퍼센트·전부 매도·수수료·거래세(채널 공개, 동일) |
...
| coin_achievement_rewards | 업적 보상 1회 수령 기록(PK: 길드·유저·`achievement_key`, `reward_amount`, `claimed_at`) |
...
`STOCK_TRADING_HOURS_ENABLED=true`이면 **KST 평일**이고 `STOCK_TRADING_START_KST` 이상 `STOCK_TRADING_END_KST` **미만**일 때만 `/매수`·`/매도` 명령이 진행된다(토·일 불가, 공휴일 미반영). `/주식목록`, `/시세`, `/주식자산` 등 조회는 제한하지 않는다.

매수 시: 매수 수수료 `floor(매수금액 * STOCK_BUY_FEE_RATE)`, 총 차감 = 매수금액 + 수수료, 수량·평균 매수가는 기존과 같이 계산한다.

트랜잭션 내에서 지갑 차감, 보유 upsert, `stock_trades`에 BUY를 기록한다(`fee` 컬럼에는 매수 수수료만).

매도 시: 매도대금 `gross`에 대해 매도 수수료·거래세를 각각 내림한 뒤 합산한 금액을 `stock_trades.fee`에 저장하고, 입금액은 `gross - 합산수수료·세`. 실현손익은 **실제 입금액 − 매수 원가**이다.
...
`/매수`·`/매도` 주문 시간은 `.env`의 `STOCK_TRADING_HOURS_ENABLED` 등으로 제한할 수 있으나, **공휴일·임시 휴장은 반영하지 않으며** 토·일만 주말로 막는다.

시세 갱신 모드(`interval` / `scheduled-close`)와 주문 가능 시간은 별개 설정이다.

시즌은 관리자가 `/시즌시작`·`/시즌종료`로 선택 운영할 수 있으며, 시즌 종료만으로 유저 지갑·거래 데이터가 자동 초기화되지는 않습니다.
...
- 주문 시간 제한을 켜도 **공휴일·임시 휴장은 미반영**(토·일만 주말 처리)
- 시즌은 선택 운영이며 종료 시 지갑·주식 자동 초기화 없음

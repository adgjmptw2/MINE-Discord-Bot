...
- `mock`: 개발용 가짜 가격
- `yahoo`: 국내주 MVP 검증용 demo/delayed quote provider (공식 투자용 API 아님)
- `twelvedata`: Twelve Data API provider

**`STOCK_PRICE_REFRESH_INTERVAL_MS`**

- 기본 **300000**ms(5분)
- 너무 낮게 두면 설정에서 **최소 60000**ms 등으로 보정될 수 있음

**`TWELVE_DATA_API_KEY`**

- `twelvedata` 사용 시 필요
- `mock` / `yahoo`만 쓸 때는 불필요
...
- 이렇게 해서 **API 호출량을 줄이고**, Provider 구현만 바꿔 **데이터 소스 교체**를 쉽게 했습니다.
- **Yahoo provider**는 공식 투자용 API가 아니라 **demo/delayed quote** 수준으로 취급합니다. 운영·공개 환경에서는 정식 라이선스 Provider로 바꾸는 것을 권장합니다.
...
- **Provider 인터페이스**로 가격 데이터 소스 분리
- **캐시 기반** 시세 조회로 호출 부담 완화
- 트랜잭션 기반 **매수·매도** 처리
- **서버별 랭킹** 계산

상세 설명·데이터 모델: **[docs/stock-investing.md](docs/stock-investing.md)** · 수동 테스트 체크리스트: **[docs/stock-manual-test.md](docs/stock-manual-test.md)**

...
개발 서버 한 곳에만 슬래시를 올리려면 `.env`에 `DEV_GUILD_ID`를 넣으세요. **예전에 같은 봇 앱으로 글로벌 슬래시를 등록해 둔 적이 있으면**, 길드 전용만 올려도 Discord가 **글로벌 + 길드**를 합쳐 보여 줘서 `/247`, `/eval` 같은 옛 명령이 같이 뜹니다. 이 레포는 `DEV_GUILD_ID`가 있을 때 시작 시 **글로벌 슬래시를 비우고** 길드 것만 두도록 처리합니다. 운영에서 여러 서버에 글로벌 슬래시가 필요하면 `DEV_GUILD_ID`를 비우세요.
